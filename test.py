...
class Emitter(QThread):
    ...
    def run(self):
        while True:
            try:
                print("Inside emitter")
                text = self.child_pipe.recv()
            except EOFError:
                print("EOFError")
                break
            else:
...
class Form(QDialog):
    def __init__(self, child_queue: Queue, emitter: Emitter):
        super().__init__()
    ...
    def to_child(self):
        ...
        self.child_queue.put(self.lineedit.text().encode('utf-8'))
        self.lineedit.clear()
...
class ChildProc(Process):
    ...
    def __init__(self, to_emitter: Pipe, from_mother: Queue):
        super().__init__()
        self.daemon = True
